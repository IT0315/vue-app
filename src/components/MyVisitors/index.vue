    ...
    <Scroll
      ...
      <div class="title-wrap">
        <flexbox :gutter="0">
          ...
          <flexbox-item>
            <div class="visitor-total"><span>{{ MyTodayCount }}</span>位访客</div>
          </flexbox-item>
        </flexbox>
      </div>
      <div class="visitor-content-wrap">
        <div v-for="(item,index) in MyTodayVisitors" :key="index">
      ...
      <div :class="MyAfterVisitors.length === 0?'dn':'db'" class="visitor-after">
        <divider>以后</divider>
      </div>
      <div class="after-content-wrap">
        <div v-for="(item,index) in MyAfterVisitors" :key="index" class="after-wrap" @click="handleDetails(item)">
          <flexbox orient="vertical">
            <flexbox-item>
              <flexbox class="item-1-wrap">
                <flexbox-item><div class="visitor-name">{{ item.visitor_name }}</div></flexbox-item>
...
export default {
  ...
  methods: {
    ...
    // 获取我的访客信息
    loadRefresh () {
      if (this.MyTodayVisitors.length || this.MyAfterVisitors.length) {
      ...
      let paramsMy = {
        'params': {
          ...
          'p_role_code': sessionStorage.getItem('userRole'),
          'p_type': 'MY',
        },
      }
      fetchIndexList(paramsMy, headers)
        .then(
          (res) => {
            ...
            this.MyAfterVisitors = res.after_visitors
            this.MyTodayVisitors = res.today_visitors
            this.MyTodayCount = res.today_count
